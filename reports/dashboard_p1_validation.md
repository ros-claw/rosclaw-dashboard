...
本次 PR 在 **Dashboard → Physical Trace Viewer** P1 能力之上，进一步补齐《ROSClaw_v1.0_深入验收指南》中可在 dashboard repo 内闭环的 L0/L2/L4/L5 验收差距：系统状态聚合、MCP 工具网关、Memory 失败解释、How 恢复建议、Forge bundle 生成与校验、Practice episode 结构、Sandbox/Firewall 可观测性。
...
| **Episode 结构** | `RunSummary` / `RunDetail` 扩展 P0 episode 字段：`episode_id`、`task_id`、`trace_id`、`agent_request`、`provider_trace`、`sandbox_result`、`runtime_action`、`critic_result`、`memory_write_result`、`artifact_uri` | `apps/api/src/models/schemas.py`、`apps/api/src/services/run_indexer.py` |
...
| **MCP 工具网关** | `GET /api/mcp/tools`、`POST /api/mcp/call`，提供 `list_robots`、`list_providers`、`list_runs`、`get_run_failures`、`run_sandbox_task`、`query_memory`、`explain_failure`、`compile_asset_bundle` | `apps/api/src/routers/mcp.py` |
| **Memory 解释** | `POST /api/memory/explain` 读取运行失败、关联事件、相似历史、manifest 中的 sandbox/provider 信息，生成“发生了什么/为什么失败”的语义解释并写入 episodic memory | `apps/api/src/routers/memory.py` |
| **How 恢复** | `POST /api/how/recovery` 基于失败类型、关联事件与历史 procedural memory 生成恢复建议与参数补丁并写入 procedural memory | `apps/api/src/routers/how.py` |
| **Forge 生成/校验** | `POST /api/forge/compile`、`POST /api/forge/validate`、`GET /api/forge/bundles`、`GET /api/forge/bundles/{bundle_id}`、`POST /api/forge/bundles/{bundle_id}/validate`：生成 5 类 bundle，执行 Critic 校验（含 target 必备文件检查），通过校验后写入 staging | `apps/api/src/routers/forge.py` |
| **防火墙拦截** | `GET /api/safety/blocks` 扫描 practice runs，汇总 sandbox/firewall `BLOCK` 事件与 manifest 级 `BLOCK` 决策 | `apps/api/src/routers/safety.py` |
...
| **Sandbox Replay** | `/sandbox` 页面：列出含 sandbox 决策的练习运行，展示 decision/risk_score/reason/checks/replay_id | `apps/web/app/sandbox/page.tsx` |
| **Firewall Blocks** | `/safety` 页面新增 Firewall Blocks 面板，展示 BLOCK 事件、原因、risk、replay | `apps/web/app/safety/page.tsx` |
| **Memory Browser** | `/memory` 页面增强：按 robot/type 过滤、Explain 问答表单、相似历史展示、类型统计 | `apps/web/app/memory/page.tsx` |
| **Forge Bundle Viewer** | `/forge` 页面增强：bundle 列表搜索、bundle 详情、文件树、内容预览、校验 check  drill-down | `apps/web/app/forge/page.tsx` |
...
- `apps/web/src/lib/api.ts`：新增 `status`、`mcp`、`how`、`forge`（含 `get`）、`safety.blocks()` 客户端方法。
- `apps/web/src/components/DashboardShell.tsx`：导航栏新增 `Sandbox Replay` 入口。

## 2. 测试结果

| 测试层级 | 命令 | 结果 |
| --- | --- | --- |
| 后端单元/集成测试 | `PYTHONPATH=src pytest apps/api/tests` | **52 passed** |
| timeline-core 构建 | `pnpm --filter @rosclaw/timeline-core build` | **成功** |
| 前端类型检查 | `npx tsc -p apps/web/tsconfig.json --noEmit` | **clean** |
...
9. **Memory page 模板字面量损坏**：写入 `apps/web/app/memory/page.tsx` 时产生多余 `}`，导致 TypeScript 解析失败；已修复。
10. **Forge revalidate 不一致**：staging 后的文件不再包含原始 SDK 文本，导致重新校验时 safety 检查失败；生成 bundle 时新增 `SDK.md` 保留 SDK 原文，使 compile 与 revalidate 结果一致。
11. **新增 blocked fixture**：新增 `golden_arm_blocked` 练习运行 fixture，专门验证 `GET /api/safety/blocks` 对 manifest 级与事件级 BLOCK 的聚合。
...
- **Memory / How**：当前为基于规则的字典匹配、关联事件搜索与历史 memory 匹配；已加入相似历史检索与置信度计算，后续可接入 LLM-based 根因分析与策略生成。
- **Forge**：当前为模板生成 + 关键词 Critic 校验；已加入 target 必备文件检查，后续应与真实 `sdk_to_mcp` / Asset Forge 编译器、签名与 staging install 流程对接。
...
| L1 模块契约 | 各模块 API 契约 | **部分涉及**：runs / replay / export / status / mcp / memory / how / forge / safety API 已定义 |
| L2 Claude Code 接入 | MCP tools 可调用 ROSClaw | **已涉及**：`/api/mcp/tools` 与 `/api/mcp/call` 暴露 8 个稳定工具，Claude Code 可通过 HTTP 调用 |
| L3 单机器人任务 | 小车 PID / 机械臂 reach / 抓取 | **未涉及**：只有 mock fixture |
| L4 失败恢复与记忆 | Memory 解释失败、How 恢复、第二轮改进 | **已涉及**：`/api/memory/explain` 与 `/api/how/recovery` 基于失败事件、关联事件、相似历史生成解释与参数补丁并持久化到 Memory；UI 已提供入口 |
| L5 自扩展 | Forge 生成 bundle、Critic validation | **已涉及**：`/api/forge/compile` 生成 5 类 bundle，`/api/forge/validate` 与 `/api/forge/bundles/{id}/validate` 执行 Critic 校验（含 target 必备文件）并拦截缺少 safety/firewall 约束的 bundle，通过校验后写入 staging |
| Dashboard 验收 | 完整 trace、事件流、replay | **基本满足**：可加载练习运行、展示多轨时间轴、回放、失败跳转、导出、系统状态、MCP 调用、Memory/How 分析、Forge 生成、Sandbox Replay、Firewall Blocks |

**结论**：本次 PR 在 P1 Trace Viewer 基础上，闭环了 Dashboard 侧可实现的 L0 状态、L2 MCP 工具、L4 Memory/How、L5 Forge 验收能力，并补齐了 Practice episode 结构与 Sandbox/Firewall 可观测性。L3 真实机器人任务与物理仿真、以及 `rosclaw` CLI 本身的 L0 命令仍需要 Runtime / Embodiment / CLI 仓库后续补齐。
