...
export default function SkillsPage() {
  ...
  useEffect(() => {
    if (selectedRobot) {
      api.robots.skills(selectedRobot).then(setSkills);
    }
  }, [selectedRobot]);
      ...
      <div className="space-y-4">
            ...
            <div key={skill.id} className="bg-white rounded-lg border border-slate-200 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                  skill.status === 'ready' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                }`}>
                  {skill.status}
                </span>
              </div>
              ...
              <div className="text-xs text-slate-400">
                ...
                {skill.approval_required && ' • Approval required'}
              </div>
            </div>
          ))}
        </div>
        ...
        {skills.length === 0 && (
          <div className="bg-white rounded-lg border border-slate-200 p-8 text-center">
            <p className="text-slate-500">No skills found for this robot.</p>
          </div>
        )}
      </div>
    </DashboardShell>
